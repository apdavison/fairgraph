...
import json
from fairgraph.client_v2 import (
    KGClient,
    core,
    brainsimulation,
    electrophysiology,
...
def save_nexus_query_result(module, cls):
    ...
    local_path = os.path.join("nexus", module.__name__.split(".")[1], filename)
...
def save_kg_query_result(module, cls, query_label):
    query_id = {"simple": cls.query_id, "resolved": cls.query_id_resolved}[query_label]
    path = "{}/{}/instances?start=0&size=10&databaseScope=INFERRED".format(cls.path, query_id)
    response = client._kg_query_client.get(path)
    fix_fields(response)
    filename = "{}_list_{}_0_10.json".format(cls.__name__.lower(), query_label)
    local_path = os.path.join("kgquery", module.__name__.split(".")[1], filename)
    with open(local_path, "w") as fp:
        json.dump(response, fp, indent=4)
            ...
            for key, value in obj.items():
                ...
                if isinstance(value, (list, dict)):
                    ...
                    if key == "givenName" and "schema" not in value:
                        ...
                        (
                            obj["http://schema.org/givenName"],
                            obj["http://schema.org/familyName"],
                            obj["http://schema.org/email"],
                        ) = next(replacement_names).split(";")
                        break
