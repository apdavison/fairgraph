...
def main(schema_path, generator_path=None, ignore=[], build_docs=False):
    ...
    types_file, properties_file = VocabExtractor(
        expander.schemas, schema_path, reinit=True, current_version="v3", vocab=OPENMINDS_VOCAB
    ).extract()
    ...
    if build_docs:
        ...
        HTMLGenerator(expander.schemas, instances, current="v3", all_tags=[], all_version_branches=["v3"]).generate(
            ignore=[]
        )
